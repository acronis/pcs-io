...
#define ERR_NAME(err) [PCS_ERR_ ## err] = #err
...
static const char *const prot_err_names[] = {
	...
	ERR_NAME(LOST_LOCK),
	ERR_NAME(NOT_FOUND),
	ERR_NAME(INTERRUPTED),
	ERR_NAME(NET_ABORT),
	ERR_NAME(CONNECT_TIMEOUT),
	ERR_NAME(AUTH_TIMEOUT),
	ERR_NAME(RESPONSE_TIMEOUT),
	ERR_NAME(WRITE_TIMEOUT),
	ERR_NAME(CANCEL_REQUEST),
	ERR_NAME(CANCEL_IO),
	ERR_NAME(LEASE_REQUIRED),
	ERR_NAME(LEASE_EXPIRED),
	ERR_NAME(LEASE_CONFLICT),
	ERR_NAME(INV_PATH),
	ERR_NAME(NOT_DIR),
	ERR_NAME(IS_DIR),
	ERR_NAME(NON_EMPTY_DIR),
	ERR_NAME(ZERO_CHUNK),
	ERR_NAME(INVALID),
	ERR_NAME(INV_PARAMS),
	ERR_NAME(NO_ID),
	...
	ERR_NAME(NORES),
	ERR_NAME(UNAVAIL),
	ERR_NAME(BAD_CLUSTER),
	ERR_NAME(READONLY),
	ERR_NAME(PERM),
	ERR_NAME(UNSUPPORTED),
	ERR_NAME(TEMP_UNAVAIL),
	ERR_NAME(INTEGRITY),
	ERR_NAME(INTEGRITY_FAIL),
	ERR_NAME(NO_STORAGE),
	ERR_NAME(NOT_ALLOWED),
	ERR_NAME(CFG_VERSION),
	ERR_NAME(CLNT_VERSION),
	ERR_NAME(EXISTS),
	ERR_NAME(EPOCH_MISMATCH),
	ERR_NAME(GR_EPOCH_INVALID),
	ERR_NAME(NO_DIR),
	ERR_NAME(DIR_INST_VER),
	ERR_NAME(INST_EXISTS),
	ERR_NAME(INST_OUTDATED),
	ERR_NAME(INST_MISMATCH),
	ERR_NAME(CONTEXT_LOST),
	ERR_NAME(CSD_STALE_MAP),
	ERR_NAME(CSD_RO_MAP),
	ERR_NAME(CSD_WR_IN_PROGR),
	ERR_NAME(CSD_REPLICATING),
	ERR_NAME(CSD_STALLED_REPL),
	ERR_NAME(CANCEL_KEEPWAIT),
	ERR_NAME(CSD_LACKING),
	ERR_NAME(CSD_DROPPED),
	ERR_NAME(MDS_NOT_MASTER),
	ERR_NAME(MDS_EXIST),
	ERR_NAME(MDS_RM_TOOMANY),
	ERR_NAME(MDS_KVSTORE_GEN_OUTDATED),
	ERR_NAME(LICENSE_LIMIT),
	ERR_NAME(NO_LICENSE),
	ERR_NAME(SSL),
	ERR_NAME(SSL_CERTIFICATE_REVOKED),
	ERR_NAME(SSL_CERTIFICATE_EXPIRED),
	ERR_NAME(SSL_UNKNOWN_CA),
	ERR_NAME(PEER_CERTIFICATE_REJECTED),
};
...
void pcs_log_syserror(int level, int err, const char *fmt, ...)
{
	va_list va;
	va_start(va, fmt);
	char *str = pcs_xvasprintf(fmt, va);
	va_end(va);
	...
	pcs_log(level, "%s; error %d (%s)", str, err, buf);
	pcs_free(str);
}
