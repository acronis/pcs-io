...
static inline int rio_parse_hdr(char *buf, char **payload, int *payload_size, int *credits, struct pcs_remote_buf **rb, struct pcs_rdma_ack **rack)
{
	struct pcs_rdmaio_hdr *hdr = (struct pcs_rdmaio_hdr *)buf;

	if (hdr->magic != RIO_MAGIC) {
		pcs_log(LOG_ERR, "wrong rio msg magic: 0x%x", hdr->magic);
		return -1;
	}

	if (hdr->version != RIO_VERSION) {
		pcs_log(LOG_ERR, "wrong rio msg version: 0x%x", hdr->version);
		return -1;
	}

	if (hdr->type >= RIO_MAX_TYPE_VALUE) {
		pcs_log(LOG_ERR, "wrong rio msg type: 0x%x", hdr->type);
		return -1;
	}

	if (hdr->size > RIO_MSG_SIZE) {
		pcs_log(LOG_ERR, "wrong rio msg size: 0x%x", hdr->size);
		return -1;
	}

	if (hdr->credits > RIO_QUEUE_DEPTH) {
		pcs_log(LOG_ERR, "wrong rio msg credits: 0x%x", hdr->credits);
		return -1;
	}

	if (hdr->type == RIO_MSG_RDMA_READ_REQ &&
	    hdr->size - sizeof(*hdr) < sizeof(struct pcs_remote_buf)) {
		pcs_log(LOG_ERR, "short rdma read req: 0x%x", hdr->size);
		return -1;
	}

	if (hdr->type == RIO_MSG_RDMA_READ_ACK &&
	    hdr->size != sizeof(*hdr) + sizeof(struct pcs_rdma_ack)) {
		pcs_log(LOG_ERR, "wrong size rdma read ack: 0x%x", hdr->size);
		return -1;
	}
